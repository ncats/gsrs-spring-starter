...
public interface KeyUserListRepository extends GsrsRepository<KeyUserList, Long> {
	...
	@Query("select distinct listName from KeyUserList where entityKey = ?1 and user_id = ?2 and kind = ?3 order by listName")	
	...
	@Modifying
	@Transactional
	@Query("delete from KeyUserList where entityKey = ?1 and user_id = ?2 and listName = ?3 and kind = ?4")
	public void removeKey(String key, Long userId, String listName, String kind);
	
	@Modifying
	@Transactional
	@Query("delete from KeyUserList where user_id = ?1 and listName = ?2 and kind = ?3")
