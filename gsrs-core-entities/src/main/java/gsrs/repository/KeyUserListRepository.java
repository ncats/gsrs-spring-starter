...
public interface KeyUserListRepository extends GsrsRepository<KeyUserList, Long> {
	...
	@Query("select distinct list_name from KeyUserList where key = ?1 and user_id = ?2 order by list_name")	
	public List<String> getAllListNamesFromKey(String key, Long userId);
			
	@Modifying
    @Transactional
    @Query("delete from KeyUserList ubsr where key = ?1 and user_id = ?2 and list_name = ?3")
