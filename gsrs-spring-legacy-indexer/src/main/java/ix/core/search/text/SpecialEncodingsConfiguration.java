...
public class SpecialEncodingsConfiguration {
   // Collection
   ...
   private static List<SpecialEncoding> specialEncodings = new ArrayList<SpecialEncoding>();
   public static  List<SpecialEncoding> getSpecialEncodings(){ return specialEncodings; }
   SpecialEncodingsConfiguration() {
      String[] replacementTokensGreek = REPLACEMENT_SOURCE_GREEK.split(";");
      for (int i = 0; i < replacementTokensGreek.length; i = i + 2) {
         specialEncodings.add(new SpecialEncoding(replacementTokensGreek[i], replacementTokensGreek[i + 1]));
      }
   }
   ...
   // Interface
   public static interface SpecialEncoder{
   ...
   // Single
   public static class SpecialEncoding implements SpecialEncoder {
